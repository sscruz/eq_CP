...
class dataset( Dataset ):
    def __init__( self, path, device):
        ...
        self.device=device
        self.control_vars=torch.Tensor( uproot.concatenate( [f"{path}:Events"], ["_input_weight_lin_ctZI/_input_weight_sm"],library='pd').values).to(self.device)
        self.weights     =torch.Tensor( uproot.concatenate( [f"{path}:Events"], ["mcWeight*mcSampleWeight*sm_weight*resampling_weight", "mcWeight*mcSampleWeight*_input_weight_lin_ctZI*resampling_weight"],library='pd').values).to(self.device) # 
